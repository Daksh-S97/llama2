...
class LLaMA:
    ...
    def text_completion(self, prompts: list[str], temperature: float = 0.6, top_p: float = 0.9, max_gen_len: Optional[int] = None):
        ...
        assert batch_size <= self.args.max_batch_size, f"batch size too large"
        max_prompt_len = max(len(prompt) for prompt in prompt_tokens)
        assert max_prompt_len <= self.args.max_seq_len, f"prompt too long"
        ...
        for cur_pos in cur_iterator:
            ...
            if temperature > 0:
                # top-p strategy after scaling logits by temperature
        ...
        for prompt_index, current_prompt_tokens in enumerate(tokens.tolist()):
            # truncate output if EOS token found
